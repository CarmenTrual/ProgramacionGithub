...
public class Uno {
  ...
  public static void barajarCartas() {
    for (int i = 0; 1 < cartas.length; i++) {
      int j = (int) (Math.random() * cartas.length);
      // cambia las cartas de posición
      Carta cartaI = cartas[i];
      Carta cartaJ = cartas[j];
      cartas[i] = cartaJ;
      cartas[j] = cartaI;
    }
  }
  ...
  public static void hayCartasEnMazo() {
    for (int i = 0; i < cartas.length; i++) { // recorre todas las cartas
      if (cartas[i] != null) { // si encuentra una carta que NO sea nula, devuelve verdadero
        return true;
      }
    }
    return false;// Si no, devuelve falso (no hay cartas en el mazo)
  }
  // -------------------------------------------------------------------------------------------------------------------------------------
  ...
  // derecha
  public static boolean robaDelMazo() {
    for (int i = 0; i < cartas.length; i++) {
      if (cartas[i] != null) { // Si encuentra una carta disponible coge la carta y la eliminamos del mazo
                               // para que no pueda ser tomada de nuevo
        Carta cartaCogida = cartas[i];
        cartas[i] = null;
        return cartaCogida; // Da la carta que se ha cogido
      }
    return null;
  }
  // -------------------------------------------------------------------------------------------------------------------------------------
