 ...
 * Contiene los métodos necesarios para manejar las cartas del jugador desde el
 * programa principal en "Uno".
...
public class Jugador {
  ...
  // cogeCarta
  // Toma la carta que se ha pasado por parámetro y la incorpora a su lista de
  // cartas, en el primer hueco libre (null)
  // -------------------------------------------------------------------------------------------------------------------------------------
  ...
  public void juegaCartaAleatoria() {
    ...
    for (int i = r; i < cartas.length; i++) { // Recorre las cartas desde la posición aleatoria hasta el final de la
                                              // lista
      if (cartas[i] != null) {
        if (cartas[i].esJugable()) { // Si es jugable, nos quedamos con esta
          ...
          cartas[i] = null;
          tapaHuecos(); // Para que no queden huecos en el array, y todas las cartas se puedan ir
                        // añadiendo a la derecha
          break;
        }
      }
    }
  ...
  // tapaHuecos
  // Tapa huecos en la lista de cartas (null), desplazando todas las que están a
  // su derecha,
